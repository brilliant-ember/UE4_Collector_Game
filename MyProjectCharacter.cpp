...
void AMyProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Collect", IE_Pressed, this, &AMyProjectCharacter::CollectPickups);
...
void AMyProjectCharacter::CollectPickups() {
	//Get all overlapping actors and put em in an array
	//for each actor we collected
		// cast the actor to a battery
		// if the cast is sucessful and the pickup is valid and active
			//call the the pickup's WasCollected function
			// Deactivate the pickup aka battery

	UE_LOG(LogTemp, Warning, TEXT("The function gets to work!"));
	...
	for (int32 iCollected = 0; iCollected < CollectedActors.Num(); ++iCollected) {
		ABattery* const TestPickup = Cast<ABattery>(CollectedActors[iCollected]);
		if (TestPickup && !TestPickup->IsPendingKill() && TestPickup->IsActive()) {
			TestPickup->wasCollected();

			//Check to see if the collected is a batteryChild
